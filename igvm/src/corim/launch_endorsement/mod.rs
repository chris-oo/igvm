...
//!
//! 1. **Stage 1 -- populate measurements.** Construct a [`LaunchEndorsement`]
//!    via [`LaunchEndorsement::for_platform`], then mark each profile-defined
//!    measurement as populated via
//!    [`set_measurement`](LaunchEndorsement::set_measurement). The digest
...
//!    [`endorse`](LaunchEndorsement::endorse) to start a [`CesTripleBuilder`]
...
//!
//! Finally, call [`build`](LaunchEndorsement::build) to consume the
...
pub enum Error {
    ...
    /// stage 1 (via [`LaunchEndorsement::set_measurement`]).
    #[error(
        "measurement {kind:?} is not populated; call \
         LaunchEndorsement::set_measurement first"
    )]
...
impl LaunchEndorsement {
    /// Start a new launch endorsement for the given platform.
    ///
    ...
    pub fn build(self) -> CorimTemplate {
        CorimTemplate::LaunchEndorsement(self)
    }
...
///
/// Created by [`LaunchEndorsement::endorse`]. The lifetime ties the
...
pub struct CesTripleBuilder<'a> {
    endorsement: &'a mut LaunchEndorsement,
    svn: u64,
...
impl CesTripleBuilder<'_> {
    ...
    ///   in stage 1 via [`LaunchEndorsement::set_measurement`].
