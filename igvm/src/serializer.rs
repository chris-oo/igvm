...
//!
//! ```rust,no_run
//! use igvm::corim::launch_endorsement::LaunchEndorsement;
//! use igvm::corim::launch_endorsement::MeasurementKind;
//! use igvm::IgvmFile;
...
//! let mut le = LaunchEndorsement::for_platform(IgvmPlatformType::SEV_SNP)?;
...
impl<'a> IgvmSerializer<'a> {
    ...
    ///
    /// For [`CorimTemplate::LaunchEndorsement`], populated measurements
    ...
    pub fn add_corim(
        ...
        let corim_bytes = match template {
            CorimTemplate::LaunchEndorsement(le) => {
                if le.platform() != platform {
                    return Err(Error::CorimGeneration(format!(
                        "LaunchEndorsement targets {:?} but add_corim was \
                         called with {platform:?}",
                        le.platform()
                    )));
                }

                self.build_launch_endorsement_corim(le)?
            }
    ...
    /// Resolve a [`LaunchEndorsement`]'s populated measurements and CES
    /// triples into the internal builder form, then build the CoRIM bytes.
    #[cfg(feature = "corim")]
    fn build_launch_endorsement_corim(
        &self,
        le: crate::corim::launch_endorsement::LaunchEndorsement,
    ) -> Result<Vec<u8>, Error> {
        use crate::corim::launch_endorsement::builder::ResolvedMeasurement;
        use crate::corim::launch_endorsement::measurement_info;
        ...
        // `LaunchEndorsement::for_platform` only succeeds for measurable
        ...
        // The current launch-endorsement builder emits exactly one
        ...
        if le.measurement_kinds().len() != 1 {
            return Err(Error::CorimGeneration(format!(
                "LaunchEndorsement requires exactly one populated measurement, got {}",
        ...
        if le.triples().len() != 1 {
            return Err(Error::CorimGeneration(format!(
                "LaunchEndorsement requires exactly one CES triple, got {}",
                le.triples().len()
            )));
        }
        ...
        crate::corim::launch_endorsement::builder::build_corim_bytes(
            platform,
            &resolved,
            triple.svn(),
        )
...
mod tests {
    ...
    /// Helper: build a `CorimTemplate::LaunchEndorsement` with a single
    /// CES triple binding the launch measurement -> svn.
    fn launch_endorsement_template(platform: IgvmPlatformType, svn: u64) -> CorimTemplate {
        use crate::corim::launch_endorsement::LaunchEndorsement;
        use crate::corim::launch_endorsement::MeasurementKind;
        let mut le = LaunchEndorsement::for_platform(platform).unwrap();
    ...
    fn add_corim_produces_larger_output() {
        ...
        serializer
            .add_corim(
                ...
                launch_endorsement_template(IgvmPlatformType::SEV_SNP, 1),
            )
            .unwrap();
    ...
    fn add_corim_uses_eager_measurement() {
        ...
        serializer
            .add_corim(
                ...
                launch_endorsement_template(IgvmPlatformType::TDX, 5),
            )
            .unwrap();
    ...
    fn add_corim_output_roundtrips() {
        ...
        serializer
            .add_corim(
                IgvmPlatformType::SEV_SNP,
                launch_endorsement_template(IgvmPlatformType::SEV_SNP, 42),
            )
            .unwrap();
    ...
    #[test]
    fn launch_endorsement_unsupported_platform() {
        use crate::corim::launch_endorsement::Error as LeError;
        use crate::corim::launch_endorsement::LaunchEndorsement;
    ...
    #[test]
    fn launch_endorsement_select_unpopulated_rejected() {
        use crate::corim::launch_endorsement::Error as LeError;
        use crate::corim::launch_endorsement::LaunchEndorsement;
        use crate::corim::launch_endorsement::MeasurementKind;
    ...
    #[test]
    fn launch_endorsement_duplicate_selection_rejected() {
        use crate::corim::launch_endorsement::Error as LeError;
        use crate::corim::launch_endorsement::LaunchEndorsement;
        use crate::corim::launch_endorsement::MeasurementKind;
        let mut le = LaunchEndorsement::for_platform(IgvmPlatformType::SEV_SNP).unwrap();
    ...
    #[test]
    fn launch_endorsement_empty_selection_rejected() {
        use crate::corim::launch_endorsement::Error as LeError;
        use crate::corim::launch_endorsement::LaunchEndorsement;
        use crate::corim::launch_endorsement::MeasurementKind;
        let mut le = LaunchEndorsement::for_platform(IgvmPlatformType::SEV_SNP).unwrap();
    ...
    fn add_corim_platform_mismatch_rejected() {
        let file = make_snp_file();
        let mut serializer = IgvmSerializer::new(&file).unwrap();
        // LaunchEndorsement targets TDX but we call add_corim with SEV_SNP.
        let template = launch_endorsement_template(IgvmPlatformType::TDX, 1);
        let err = serializer
